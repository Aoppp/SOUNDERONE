...
### Agent 核心能力改造（路由、实体、语义检索、门控与事实校验）

- 将检索从意图路由后的被动步骤提升为路由证据：产品实体、业务线索、FAQ标题近似和轻量混合检索候选共同决定是否进入RAG，解决“知识存在但有限关键词先否决”的结构问题。
- 新增 `ProductEntityResolver`，统一规范产品全名、昵称、省略剂型和缩写，首批覆盖 AM/氨基酸洗发水、VCIP、B5、EUK-134、双A醇及木火水土洗发水；实体在查询改写前归一化。
- 增加上下文存在性约束：无历史实体时，“这个/这些/这几款/它们”进入澄清；复数指代要求会话中至少存在两个产品，禁止 LLM 自行构造指代对象。
- 默认 Dense Embedding 从 Hash 替换为本地 FastEmbed `BAAI/bge-small-zh-v1.5`（512维中文语义模型）；Hash仅供确定性测试，OpenAI仍作为可选 provider。模型已在独立环境真实下载和加载，向量维度校验通过。
- 将相关性阈值拆分为 FAQ、普通产品知识、推荐/对比/搭配和路由候选四类配置；保留候选分数窗口。用新中文向量实测 FAQ、缩写、推荐、对比、范围外正负样本并校准。
- 新增确定性 grounding verifier：DeepSeek生成后的数值、产品实体及关键功效/肤质事实必须存在于检索引用，失败以 `unsupported_generated_claim` 转人工；不会把不受支持的答案直接发给客户。
- 新增7项核心回归，确定性全量结果由此前 `114 passed, 1 failed` 提升到 `122 passed`；已知 `FAQ-009` 通过。真实中文向量+DeepSeek验证：B5混合问候、AM质地FAQ、VCIP无用法转人工、无上下文复数指代、范围外、美白多轮主流程均符合新路由设计。
- 本轮明确不处理 FAQ 原始话术清洗和抖音官方生产接入，保持用户指定的第7、8项待定。
