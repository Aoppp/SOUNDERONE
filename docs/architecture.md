...
- `conversation topic`：每次成功回答后保存当前主题查询、业务意图、已引用文档和所有已回答产品别名。“还有其他的吗”会继承主题并排除已回答文档/产品；“这些都可以吗/它们有什么区别”会把历史引用以 `conversation_context` 通道带回生成节点。单产品“这个怎么用”仍优先使用已确认的产品实体和当前用法意图。
